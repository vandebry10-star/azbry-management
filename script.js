...
todayRows.forEach((t) => {
tbody.innerHTML +=   <tr>   <td class="${t.type === "income" ? "green" : "red"}">   ${formatRupiah(t.amount)}   </td>   <td>${t.type === "income" ? "Masuk" : "Keluar"}</td>   <td>${t.note || "-"}</td>   <td>   <button class="btn-delete" onclick="deleteTxn(${t.id})">Hapus</button>   </td>   </tr>  ;
});
}
...
rows.forEach((t) => {
tbody.innerHTML +=   <tr>   <td>${t.date}</td>   <td>${   t.type === "income"   ? '<span class="green">Masuk</span>'   : '<span class="red">Keluar</span>'   }</td>   <td>${t.category || "-"}</td>   <td>${t.note || "-"}</td>   <td class="${t.type === "income" ? "green" : "red"}">   ${formatRupiah(t.amount)}   </td>   <td>   <button class="btn-delete" onclick="deleteTxn(${t.id})">Hapus</button>   </td>   </tr>  ;
});
}
